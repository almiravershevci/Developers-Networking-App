...
class DashboardViewModel(
                ...
                _uiState.update {
                    it.copy(
                        ...
                        feedPosts = content.projectPosts.mapIndexed { index, post ->
                            createFeedPostState(post = post, id = post.projectId.ifBlank { "${post.title.hashCode()}-$index" })
                        },
                        errorMessage = null,
                    )
                }
    ...
    fun sendProjectJoinRequest(
        ...
        viewModelScope.launch {
            _uiState.update { it.copy(projectJoinActionInFlight = projectId) }
            ...
            result.fold(
                onSuccess = {
                    repository.invalidateDashboard()
                    notify("✓ Join request sent to the project owner. You'll be notified when they respond.")
                },
                onFailure = { notify(it.message ?: "Couldn't send join request.") },
            )
        }
    }
